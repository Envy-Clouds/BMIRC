...
# --------------------------------------------------------
import json
from utils.Optimizer_Linear import SGD
import torch
...
def get_num_layer_for_vit(var_name, num_max_layer):
    ...
    elif var_name.startswith("encoder_ms"):
        layer_id = int(var_name.split('.')[1])
        return layer_id + 1
    elif var_name.startswith("ms_t_norm") or var_name.startswith("ms_f_norm") or var_name.startswith("transfom_norm"):
        return 4
    elif var_name.startswith("encoder_mc"):
        layer_id = int(var_name.split('.')[1])
        return layer_id + 5
...
#     elif var_name.startswith("rel_pos_bias"):
#         return num_max_layer - 1
#     elif var_name.startswith("ms_ecg_norm"):
#         return 4
#     elif var_name.startswith("ms_pcg_norm"):
#         return 4
#     elif var_name.startswith("encoder"):
#         layer_id = int(var_name.split('.')[1])
#         return layer_id + 1
...
def create_optimizer(args, model, get_num_layer=None, get_layer_scale=None, filter_bias_and_bn=True, skip_list=None):
    ...
    if opt_lower == 'sgd' or opt_lower == 'nesterov':
        ...
        optimizer = SGD(parameters, lr=0.001, gammas=[0.9, 0.5], momentum=0.9, accumulated_step=5,
                        last_step=30*8194)
